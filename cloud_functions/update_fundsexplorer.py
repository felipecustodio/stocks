...
async def get_info_table():
    browser = await launch(options={'args': ['--no-sandbox']})
    page = await browser.newPage()
    await page.goto(url)
    html = await page.frames[1].content()
    await browser.close()
    return html
...
def update_fundsexplorer(request):
    ...
    print("Reading funds info...")
    try:
        html = asyncio.get_event_loop().run_until_complete(get_info_table())
        funds_info = pd.read_html(html, thousands=None, encoding='utf-8')[0]
    except Exception as e:
        print(e)
        return str(e)
    ...
    try:
        ...
        funds = pd.read_html(html_ranking, thousands=None, encoding='utf-8')[0]
    ...
    print("Merging info...")
    try:
        funds['Código'] = funds['Código do Fundo'].apply(
            lambda x: x.split('11')[0])
        funds = (funds.merge(
            funds_info, left_on="Código", right_on="Código"))
    except Exception as e:
        print(e)
        return str(e)

    print("Data cleanup...")
    string_columns = ["Código do Fundo", "Código",
                      "Razão Social", "Setor", "Segmento", "Fundo"]
    for column in funds:
        if (column not in string_columns):
            try:
                ...
                funds[column] = funds[column].apply(unidecode.unidecode)
            except Exception as e:
                print(e)
                return str(e)

    funds = funds.astype(str)
    funds = funds.replace('nan', '')
    funds = funds.replace('NaN', '')
