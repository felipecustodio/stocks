...
def update_magicformula(request):
    print("Initializing Storage...")
    try:
        storage_client = storage.Client()
        bucket = storage_client.get_bucket('carteiras')
        portfolio = bucket.blob('portfolio_magicformula.json')
    ...
    print("Rank by ROIC...")
    try:
        filtered_stocks.sort_values("ROIC", axis=0, ascending=False,
                                 inplace=True, kind="mergesort", na_position="last")
        filtered_stocks["Rank ROIC"] = filtered_stocks["ROIC"].rank(method="max")
    except Exception as e:
        print(e)
        return str(e)
    ...
    try:
        filtered_stocks["MagicFormula"] = (
            filtered_stocks["Rank EV/Ebit"] + filtered_stocks["Rank ROIC"]).astype(int)
        filtered_stocks.sort_values("MagicFormula", axis=0, ascending=True,
                                 inplace=True, kind="mergesort", na_position="last")
    except Exception as e:
        print(e)
        return str(e)
    ...
    try:
        updated_portfolio = filtered_stocks.head(
            30)[["Papel", "Tipo", "Empresa", "Setor", "MagicFormula"]]
